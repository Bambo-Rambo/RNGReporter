    ...
    internal class FrameGenerator
    {
            ...
            uint sid)
        {
            ...
            else if (frameType == FrameType.Method5Natures)
            {
                ...
                bool SyncGenerator = EncounterMod == EncounterMod.Synchronize;
                bool CuteCharmGenerator = EncounterMod == EncounterMod.CuteCharm;
                bool CompEyesGenerator = EncounterMod == EncounterMod.Compoundeyes;
                bool SuctionCupsGenerator = EncounterMod == EncounterMod.SuctionCups;

                BWRng rngTrigger = new BWRng(InitialSeed);
                BWRng rngTrigger2 = new BWRng(InitialSeed);
                ...
                if (!(G5_WildFishingSpot && SuctionCupsGenerator))
                    LevelRand++;
                ...
                !(G5_WildSuperRod && SuctionCupsGenerator))
                    LevelRand++;
                ...
                {
                    rngTrigger.GetNext64BitNumber();
                    rngTrigger2.GetNext64BitNumber();
                }
                ...
                for (uint cnt = 0; cnt < maxResults; cnt++, rngList.RemoveAt(0), rngList.Add(rng64.GetNext32BitNumber()))
                {
                    ...
                    uint pid;
                    bool synchable;
                    LvlRand = LevelRand;    //Restore value
                    ...
                    else
                    {
                        if (G5_Wild || G5_WildSurfing || G5_WildWaterSpot || G5_WildFishingSpot)
                        {
                            ...
                            if (TimeFinder5)
                            {
                                ...
                                if (((((ulong)rngList[0] * 0xFFFF) >> 32) / 656) < 67)      //Cute Charm Success
                                {
                                    encounterSlot = EncounterSlotCalc.encounterSlot(rngList[1], frameType, EncounterType, isBW2);
                                    CuteCharmModify(frameCompare, id, sid, idLower, true, encounterSlot, 3, cnt + InitialFrame, 3, LvlRand, CurrentRatio);
                                }
                                /*else
                                {
                                    //Wrong. Also not needed for these encounter types

                                    encounterSlot = EncounterSlotCalc.encounterSlot(rngList[2], frameType, EncounterType, isBW2);
                                    CuteCharmModify(frameCompare, id, sid, idLower, false, encounterSlot, 0, cnt + InitialFrame, 3, LvlRand + 1, CurrentRatio);
                                }*/

                                encounterSlot = EncounterSlotCalc.encounterSlot(rngList[1], frameType, EncounterType, isBW2);
                                pid = FindPID(id, sid, idLower, 3);
                                synchable = CheckLead(rngTrigger, false, 0);
                            ...
                            else if (CuteCharmGenerator)
                            {
                                ...
                                if (!synchable)
                                {
                                    LvlRand++;
                                    encounterSlot = EncounterSlotCalc.encounterSlot(rngList[2], frameType, EncounterType, isBW2);
                                    pid = FindCuteCharmPID(id, sid, idLower, false, 4, SynchNature);
                                    nature = (uint)(((ulong)rngList[TotalRandCalls + 4] * 25) >> 32);
                                }
                                else
                                {
                                    encounterSlot = EncounterSlotCalc.encounterSlot(rngList[1], frameType, EncounterType, isBW2);
                                    pid = FindCuteCharmPID(id, sid, idLower, true, 3, SynchNature);
                                    nature = (uint)(((ulong)rngList[TotalRandCalls + 3] * 25) >> 32);
                                }

                                synchable = false;
                            }
                            else if (CompEyesGenerator || SuctionCupsGenerator)
                            {
                                pid = FindPID(id, sid, idLower, 2);

                                synchable = false;
                                encounterSlot = EncounterSlotCalc.encounterSlot(rngList[0], frameType, EncounterType, isBW2);

                                nature = (uint) (((ulong) rngList[TotalRandCalls + 2] *25) >> 32);

                                //This is necessary to show correct results but Suction Cups is not being used in water much
                                if (G5_WildWaterSpot || G5_WildSurfing)
                                    LvlRand--;
                        ...
                        else if (G5_WildCaveSpot)
                        {
                            ...
                            if (TimeFinder5)
                            {
                                ...
                                frame = Frame.GenerateFrame(FrameType.Method5Natures, EncounterType, cnt + InitialFrame, rngList[0], pid, id, sid, nature,
                                    false, encounterSlot, getLevel(rngList[LvlRand]), item, 0, false);
                                ...
                                if (((((ulong)rngList[1] * 0xFFFF) >> 32) / 656) < 67)      //Cute Charm Success
                                {
                                    encounterSlot = EncounterSlotCalc.encounterSlot(rngList[2], frameType, EncounterType, isBW2);
                                    CuteCharmModify(frameCompare, id, sid, idLower, true, encounterSlot, 4, cnt + InitialFrame, 4, LvlRand, CurrentRatio);
                                }


                                encounterSlot = EncounterSlotCalc.encounterSlot(rngList[2], frameType, EncounterType, isBW2);
                                pid = FindPID(id, sid, idLower, 4);
                            ...
                            else if (CuteCharmGenerator)
                            {
                                ...
                                if (battle)
                                    encounterSlot = EncounterSlotCalc.encounterSlot(rngList[synchable ? 2 : 3], frameType, EncounterType, isBW2);
                                else
                                    encounterSlot = FindItem();  

                                if (!synchable)     //Cute Charm failed
                                {
                                    pid = FindCuteCharmPID(id, sid, idLower, false, 5, SynchNature);
                                    nature = (uint)(((ulong)rngList[TotalRandCalls + 5] * 25) >> 32);
                                }
                                else
                                {
                                    pid = FindCuteCharmPID(id, sid, idLower, true, 4, SynchNature);
                                    nature = (uint)(((ulong)rngList[TotalRandCalls + 4] * 25) >> 32);
                                }
                        ...
                        else if (G5_WildSwarm)
                        {
                            if (TimeFinder5)
                            {
                                ...
                                if (CheckLead(rngTrigger2, true, 0))      //Cute Charm Success
                                {
                                    encounterSlot = DoubleEnc_Swarm(1) ? 12 : EncounterSlotCalc.encounterSlot(rngList[2], frameType, EncounterType, isBW2);
                                    CuteCharmModify(frameCompare, id, sid, idLower, true, encounterSlot, 4, cnt + InitialFrame, 4, LvlRand, CurrentRatio);
                                }

                                synchable = CheckLead(rngTrigger, false, 0);
                                // Swarm check for slot
                                encounterSlot = DoubleEnc_Swarm(1) ? 12 : EncounterSlotCalc.encounterSlot(rngList[2], frameType, EncounterType, isBW2);
                                pid = FindPID(id, sid, idLower, 4);
                                nature = (uint)(((ulong)rngList[TotalRandCalls + 4] * 25) >> 32);
                                if (synchable && !frameCompare.CompareNature(nature))
                            ...
                            {
                                synchable = CheckLead(rngTrigger, true, 0);

                                if (!synchable)     // Cute Charm failed
                                {
                                    LvlRand++;
                                    CurrentRatio = getRatio(1);
                                    encounterSlot = DoubleEnc_Swarm(2) ? 12 : EncounterSlotCalc.encounterSlot(rngList[3], frameType, EncounterType, isBW2);
                                    pid = FindCuteCharmPID(id, sid, idLower, false, 5, SynchNature);
                                    nature = (uint)(((ulong)rngList[TotalRandCalls + 5] * 25) >> 32);
                                }
                                else
                            ...
                            else if (CompEyesGenerator)     //Pointless?
                            {
                                // Swarm check for slot
                                encounterSlot = DoubleEnc_Swarm(0) ? 12 : EncounterSlotCalc.encounterSlot(rngList[1], frameType, EncounterType, isBW2);
                                synchable = false;
                        ...
                        else if (G5_WildDarkGrass)
                        {
                            if (TimeFinder5)
                            {
                                ...
                                if (CheckLead(rngTrigger, true, 0))      //Cute Charm Success
                                {
                                    DoubleEncounter = DoubleEnc_Swarm(SearchForTrigger ? 0 : 1);

                                    encounterSlot = EncounterSlotCalc.encounterSlot(rngList[2], frameType, EncounterType, isBW2);
                                    CuteCharmModify(frameCompare, id, sid, idLower, true, encounterSlot,
                                        DoubleEncounter ? 6 : 4, cnt + InitialFrame, DoubleEncounter ? 6 : 4, LvlRand, getRatio(1));
                                }


                                if (SearchForTrigger)
                                {
                                    DoubleEncounter = DoubleEnc_Swarm(0); 
                                    CurrentRatio = getRatio(1);
                                }
                                else
                                    DoubleEncounter = DoubleEnc_Swarm(1);

                                synchable = CheckLead(rngTrigger, false, 0);
                                encounterSlot = EncounterSlotCalc.encounterSlot(rngList[2], frameType, EncounterType, isBW2);
                                pid = FindPID(id, sid, idLower, DoubleEncounter ? 6 : 4);
                            ...
                            else if (CuteCharmGenerator)
                            {
                                ...
                                if (!synchable)     // Cute Charm failed
                                {
                                    ...
                                    {
                                        DoubleEncounter = DoubleEnc_Swarm(1);
                                        CurrentRatio = getRatio(2);
                                    }
                                    else
                                    {
                                        DoubleEncounter = DoubleEnc_Swarm(2);
                                    }
                                    encounterSlot = EncounterSlotCalc.encounterSlot(rngList[3], frameType, EncounterType, isBW2);
                                    pid = FindCuteCharmPID(id, sid, idLower, false, DoubleEncounter ? 7 : 5, SynchNature);
                                    nature = (uint)(((ulong)rngList[TotalRandCalls + (DoubleEncounter ? 7 : 5)] * 25) >> 32);
                                }
                                else
                                {
                                    if (SearchForTrigger)
                                    ...
                                    encounterSlot = EncounterSlotCalc.encounterSlot(rngList[2], frameType, EncounterType, isBW2);
                                    pid = FindCuteCharmPID(id, sid, idLower, true, DoubleEncounter ? 6 : 4, SynchNature);
                                    nature = (uint)(((ulong)rngList[TotalRandCalls + (DoubleEncounter ? 6 : 4)] * 25) >> 32);
                                }
                        ...
                        else if (G5_Stationary)
                        {
                            if (TimeFinder5)
                            {
                                if (((((ulong)rngList[0] * 0xFFFF) >> 32) / 656) < 67)      //Cute Charm Success
                                {
                                    CuteCharmModify(frameCompare, id, sid, idLower, true, encounterSlot, 1, cnt + InitialFrame, 1, LvlRand, CurrentRatio);
                                }

                                synchable = (rngList[0] >> 31) == 1;
                                pid = FindPID(id, sid, idLower, 1);
                            ...
                            else if (CuteCharmGenerator)
                            {
                                ...
                                if (!synchable)
                                {
                                    pid = FindCuteCharmPID(id, sid, idLower, false, 2, SynchNature);
                                    nature = (uint)(((ulong)rngList[TotalRandCalls + 2] * 25) >> 32);
                                }
                                else
                                {
                                    pid = FindCuteCharmPID(id, sid, idLower, true, 1, SynchNature);
                                    nature = (uint)(((ulong)rngList[TotalRandCalls + 1] * 25) >> 32);
                                }

                                synchable = false;
                            }
                            else if (CompEyesGenerator)
                            {
                                synchable = false;
                                nature = (uint) (((ulong) rngList[1]*25) >> 32);
                        ...
                        else
                        {
                            ...
                            if (TimeFinder5)
                            {
                                if (G5_WildSuperRod)
                                {
                                    if ((rngList[1] >> 16) / 656 < 50)      //Successful Fishing encounter trigger
                                    {
                                        synchable = (rngList[0] >> 31) == 1;
                                        pid = FindPID(id, sid, idLower, 4);
                                        nature = (uint)(((ulong)rngList[TotalRandCalls + 4] * 25) >> 32);
                                        ...
                                        frame = Frame.GenerateFrame(FrameType.Method5Natures, EncounterType, cnt + InitialFrame, rngList[0],
                                            pid, id, sid, nature, synchable, encounterSlot, getLevel(rngList[LvlRand]), item, 0, false);
                                    ...
                                    frame = Frame.GenerateFrame(FrameType.Method5Natures, EncounterType, cnt + InitialFrame, rngList[0],
                                        pid, id, sid, nature, false, encounterSlot, getLevel(rngList[LvlRand - 1]), item, 0, false);
                            ...
                            {
                                // not a synch, but the Cute Charm check. Finished by Bambo_Rambo
                                synchable = ((((ulong)rngList[G5_WildSuperRod ? 0 : 1] * 0xFFFF) >> 32) / 656) < 67;
                            ...
                            {
                                if (G5_WildSuperRod && (rngList[0] >> 16) / 656 >= 50)
                                    continue;
        ...
        private void CuteCharmModify(FrameCompare frameCompare, uint id, uint sid, uint idLower, 
            bool CCSuccess, int encounterSlot, int pidRand, uint CurrentFrame, int natureRand, int LvlRand, ulong CurrentRatio)
        {
            if (CCSuccess)
            {
                ...
                for (int i = -4; i < 5; i++)
                {
                    ...
                    frame = Frame.GenerateFrame(FrameType.Method5Natures, EncounterType,
                        CurrentFrame,                                                           //cnt + InitialFrame
                        rngList[0],
                        FindCuteCharmPID(id, sid, idLower, CCSuccess, pidRand, i),              //PID. CCSuccess is true always here, 
                        id, 
                        sid,
                        (uint)(((ulong)rngList[TotalRandCalls + natureRand] * 25) >> 32),       //Nature
            ...
            else
            {
                frame = Frame.GenerateFrame(FrameType.Method5Natures, EncounterType, 
                    CurrentFrame, 
                    rngList[0],
                    FindCuteCharmPID(id, sid, idLower, CCSuccess, pidRand, 0), //Check again
                    id, 
                    sid,
                    (uint)(((ulong)rngList[TotalRandCalls + natureRand] * 25) >> 32),
                    false,
                    encounterSlot, 
        ...
        //Double Encounter decision and Swarm slot share the exact same formula
        private bool DoubleEnc_Swarm(int randCall)
        {
            return (((ulong)rngList[randCall]) * 100 >> 32) < 40;
        }

        private bool CheckLead(BWRng rng, bool CC, int randCall)
        {
            if (SearchForTrigger)
            {
                if (CC)     //Cute Charm check
                    return (((ulong)rng.GetNext32BitNumber() * 0xFFFF) >> 32) / 656 < 67;
                else
                    return (rng.GetNext32BitNumber() >> 31) == 1;
            }
            else
            {
                if (CC)     //Cute Charm check
                    return (((ulong)rngList[randCall] * 0xFFFF) >> 32) / 656 < 67;
                else
                    return (rngList[randCall] >> 31) == 1;
            }
        }

        private uint FindPID(uint id, uint sid, uint idLower, int RandCall)
        {
            uint pid = 0;
            if (ShinyCharm)
            {
                ...
                for (int n = 0; n < 3; n++)
                {
                    ...
                    pid = rngList[n + RandCall] ^ 0x10000;
            ...
            {
                TotalRandCalls = 1;
                pid = rngList[RandCall] ^ 0x10000;
                return TIDBitTwiddle(idLower, pid);
            }
                
        }
        private uint FindCuteCharmPID(uint id, uint sid, uint idLower, bool CC_Success, int RandCall, int GenderCase)
        {
            uint pid = 0;
            if (ShinyCharm)
            {
                ...
                for (int n = 0; n < 3; n++)
                {
                    pid = rngList[TotalRandCalls + RandCall] ^ 0x10000;

                    if (CC_Success)
                    {
                        pid = Functions.GenderModPID(pid, rngList[TotalRandCalls + (RandCall + 1)], GenderCase);
                        TotalRandCalls += 2;
                    }
                    else
                    {
            ...
            else
            {
                ...
                if (CC_Success)
                {
                    pid = Functions.GenderModPID(pid, rngList[RandCall + 1], GenderCase);
                    TotalRandCalls = 2;
                }
                else
                {
        ...
        //Item calculation is wrong, maybe check later
